...
import 'package:app_tcareer/src/features/jobs/chat/presentation/controllers/job_conversation_controller.dart';
...
class JobChatController extends ChangeNotifier {
  final JobChatUseCase chatUseCase;
  final Ref ref;

  JobChatController(this.chatUseCase, this.ref) {
    // listenMessage();
  }
  ...
  Future<void> markReadMessage(
    ...
    final userUtil = ref.watch(userUtilsProvider);
    String clientId = await userUtil.getUserId();
    final conversationController = ref.read(jobConversationControllerProvider);
